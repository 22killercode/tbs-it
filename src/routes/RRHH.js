...
router.post('/dataPartner',async (req, res) => {
    ...
    const {Nombre, Apellido, NumCel, Email, tipoPartner, nivelIngles, MonedaCobro, PaisResi, CiudadResi, Empresa, ModTrab, linke,tecnoBackend, tecnoFrontend} = req.body;
    const {Archivo} = req.files;
    const nombreAdjunto = []
    //Genera Token de seguridad en BD
    const token = shortid.generate()
    ...
    if(!req.files || Object.keys(req.files).lehgth === 0  ) { 
        ...
        return;
            ...
            if (extValidas.includes(extension)) {
                ...
                // guardar en BD
                const guardar = new talentos ({partner,Nombre, Apellido, NumCel, Email, tipoPartner, nivelIngles, MonedaCobro, PaisResi, CiudadResi, Empresa, ModTrab, linke,tecnoBackend, tecnoFrontend, uploadPath})
                guardar.save()
