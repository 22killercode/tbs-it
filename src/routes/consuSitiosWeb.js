...
router.post('/mensajedecontactnuestrosclient/claveCliente/puebloiruya', async (req, res) => {
    ...
    try {
        ...
        // Buscar el email del destinatario y el email de salida (ejemplos, debes reemplazarlos con tu lógica)
        const avisoDEmail = ['edgargustavovargas@gmail.com','EdgarGustavovargas@gmail.com','info@puebloiruya.com'];
        const emailSalida = 'info@puebloiruya.com';

        const transporter = nodemailer.createTransport({
            host: "mail.puebloiruya.com",
            port: 465,
            secure: true, // use TLS
            auth: {
                user: "info@puebloiruya.com",
                pass: "GustavoOrange22",
            },
            tls: {
                // do not fail on invalid certs
                rejectUnauthorized: false,
...
`
        ...
        //res.status(200).json({ mensaje: 'Correo enviado exitosamente' });
        res.redirect("https://puebloiruya.com/#mensajeEnvio");
    } catch (error) {
        // Responder con un código 404 si algo salió mal
        res.status(404).json({ error: 'Error al enviar el correo', error });
    }
